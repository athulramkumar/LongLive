...
Run with: pytest gradio/tests/test_web_demo.py -v

Note: These tests mock the gradio module since it's not available in the test environment.
...
# Create comprehensive gradio mock BEFORE any imports
mock_gr = MagicMock()
...
# Mock gradio as 'gr' which is what web_demo uses
sys.modules['gradio'] = mock_gr

# Import web_demo directly using importlib to avoid path conflicts
import importlib.util
web_demo_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'web_demo.py')
