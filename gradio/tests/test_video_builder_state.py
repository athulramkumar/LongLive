...
from gradio.video_builder_state import PromptEnhancer, VideoBuilderState, ChunkState
...
class TestPromptEnhancer:
    ...
    def mock_anthropic(self):
        ...
        with patch('gradio.video_builder_state.anthropic.Anthropic') as mock:
            client_instance = Mock()
            mock.return_value = client_instance
            
            # Mock the messages.create response
            response = Mock()
            response.content = [Mock(text="Enhanced prompt response")]
    ...
    def test_set_grounding(self, mock_anthropic):
        """Test setting initial grounding"""
        enhancer = PromptEnhancer(api_key="test-key")
        
        result = enhancer.set_grounding("A red car in a parking lot")
        
        assert result == "Enhanced prompt response"
        assert enhancer.grounding == "A red car in a parking lot"
        assert len(enhancer.previous_prompts) == 1
        assert enhancer.previous_prompts[0] == "Enhanced prompt response"
    
    def test_set_grounding_direct(self, mock_anthropic):
    ...
    def test_enhance_prompt_without_grounding(self, mock_anthropic):
    ...
    def test_enhance_prompt_with_grounding(self, mock_anthropic):
        """Test enhancing a prompt after grounding is set"""
        enhancer = PromptEnhancer(api_key="test-key")
        enhancer.set_grounding("A red car")
        
        # Reset mock to track new calls
        mock_anthropic.messages.create.reset_mock()
        mock_anthropic.messages.create.return_value.content = [Mock(text="Car transforms")]
        
        result = enhancer.enhance_prompt("transforms into a robot")
        
        assert result == "Car transforms"
        assert len(enhancer.previous_prompts) == 2
    
    def test_enhance_prompt_no_history(self, mock_anthropic):
        """Test enhancing without adding to history"""
        enhancer = PromptEnhancer(api_key="test-key")
        enhancer.set_grounding("A cat")
        
        mock_anthropic.messages.create.return_value.content = [Mock(text="Cat jumps")]
        
        result = enhancer.enhance_prompt("jumps", add_to_history=False)
        
        assert result == "Cat jumps"
        assert len(enhancer.previous_prompts) == 1  # Only grounding
    
    def test_add_to_history(self, mock_anthropic):
    ...
    def test_revert_to_chunk_0(self, mock_anthropic):
    ...
    def test_revert_to_chunk_1(self, mock_anthropic):
    ...
    def test_revert_to_invalid_preserves_minimum(self, mock_anthropic):
    ...
    def test_api_error_fallback(self, mock_anthropic):
        ...
        enhancer = PromptEnhancer(api_key="test-key")
        enhancer.grounding = "A bird"
        enhancer.previous_prompts = ["A bird"]
        
        mock_anthropic.messages.create.side_effect = Exception("API Error")
        
        result = enhancer.enhance_prompt("flies away", add_to_history=False)
        
        # Should return fallback combining grounding with message
        assert "A bird" in result
...
class TestIntegration:
    ...
    def test_full_flow_mocked(self):
        """Test the full flow with mocked dependencies"""
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key"}), \
             patch('gradio.video_builder_state.anthropic.Anthropic') as mock_anthropic:
            
            # Setup anthropic mock
            response = Mock()
            response.content = [Mock(text="Enhanced grounding")]
            mock_anthropic.return_value.messages.create.return_value = response
            
            # Test PromptEnhancer flow
            enhancer = PromptEnhancer(api_key="test-key")
            
            # Set grounding
            result = enhancer.set_grounding("A cat on a roof")
            assert result == "Enhanced grounding"
            assert enhancer.grounding == "A cat on a roof"
            
            # Enhance a prompt
            response.content = [Mock(text="Cat jumps down")]
            result = enhancer.enhance_prompt("it jumps down")
            assert result == "Cat jumps down"
            assert len(enhancer.previous_prompts) == 2
